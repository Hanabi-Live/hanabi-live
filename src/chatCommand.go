...
var (
	...
	chatPregameCommandMap = make(map[string]func(*Session, *CommandData, *Table))
)
...
func chatCommand(s *Session, d *CommandData, t *Table) {
	...
	if t == nil {
		...
		if _, ok := chatPregameCommandMap[command]; !ok {
			chatServerPregameSend("That is not a valid pre-game command.", t.ID)
			return
		}
		chatPregameCommandMap[command](s, d, t)
	}
}
