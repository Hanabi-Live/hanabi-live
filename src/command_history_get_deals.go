...
func commandHistoryGetDeals(s *Session, d *CommandData) {
	// Get the games played on this seed
	var gameHistoryList []*GameHistory
	if v, err := models.Games.GetAllDealsFromGameID(d.GameID); err != nil {
		logger.Error("Failed to get the other deals from the database for game "+
			strconv.Itoa(d.GameID)+":", err)
		s.Error("Failed to get the other deals for game " + strconv.Itoa(d.GameID) + ". " +
