...
const (
	// The maximum number of characters that a table name can be
	maxTableNameLength = 45
)

var (
	newTableID     = 1 // Start at 1 and increment for every table created
...
func commandTableCreate(s *Session, d *CommandData) {
	...
	*/
	...
	if shuttingDown {
		s.Warning("The server is restarting soon (when all ongoing tables have finished). " +
			"You cannot start any new tables for the time being.")
		return
	}
	...
	// Make a default table name if they did not provide one
	if len(d.Name) == 0 {
		d.Name = s.Username() + "'s table"
	}

	// Validate that the table name is not excessively long
	if len(d.Name) > maxTableNameLength {
		s.Warning("You cannot have a table name be longer than " +
			strconv.Itoa(maxTableNameLength) + " characters.")
		return
	}

	// Validate that the table name does not contain any special characters
	// (this mitigates XSS-style attacks)
	if !isAlphanumericSpacesAndSafeSpecialCharacters(d.Name) {
		s.Warning("Table names can only contain English letters, numbers, spaces, hyphens, " +
			"and exclamation marks.")
		return
	}

	// If they are trying to create a table with a special option, validate the options
	setSeed := ""
	setReplay := 0
	setReplayTurn := 0
	setDeal := ""
	if strings.HasPrefix(d.Name, "!") {
		if strings.HasPrefix(d.Name, "!seed") {
			...
			if match == nil {
				s.Warning("Tables on specific seeds must be created in the form: " +
					"!seed [seed number]")
				return
			}
			setSeed = match[1]
			...
			// !replay - Replay a specific table up to a specific turn
			...
			if match1 != nil {
				...
				s.Warning("Replays of specific tables must be created in the form: " +
					"!replay [table ID] [turn number]")
				return
			}
			...
			// Check to see if the table ID exists on the server
			if exists, err := db.Games.Exists(setReplay); err != nil {
				log.Error("Failed to check to see if table "+
					strconv.Itoa(setReplay)+" exists:", err)
				s.Error("Failed to create the table. Please contact an administrator.")
				return
			} else if !exists {
				s.Warning("That table ID does not exist in the database.")
				return
			}

			// Check to see if this turn is valid
			// (it has to be a turn before the table ends)
			var numTurns int
			if v, err := db.Games.GetNumTurns(setReplay); err != nil {
				log.Error("Failed to get the number of turns from the database for table "+strconv.Itoa(setReplay)+":", err)
			...
			if setReplayTurn >= numTurns {
				s.Warning("That turn is not valid for the specified table ID.")
				return
			}

			// Set the options of the table to be the same as the one in the database
			if v, err := db.Games.GetOptions(setReplay); err != nil {
				log.Error("Failed to get the variant from the database for table "+strconv.Itoa(setReplay)+":", err)
				s.Error("Failed to initialize the table. Please contact an administrator.")
				return
			} else {
			...
			if match == nil {
				s.Warning("Tables on specific deals must be created in the form: !deal [filename]")
				return
			}
			setDeal = match[1]
			...
			// (we won't bother parsing the file until the table is actually started)

		} else {
			s.Warning("You cannot start a table with an exclamation mark unless " +
				"you are trying to use a specific table creation command.")
			return
		}
	}
	...
	// Blank out the time controls if this is not a timed table
	if !d.Timed {
		d.BaseTime = 0
		d.TimePerTurn = 0
	}
	...
	*/
	...
	if d.AlertWaiters && t.Password == "" && !strings.HasPrefix(t.Name, "test") {
		// Even if they check the "Alert people" checkbox,
		// we don't want to alert on password-protected tables or test tables
