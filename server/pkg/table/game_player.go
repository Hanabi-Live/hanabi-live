...
func (p *GamePlayer) GiveClue(target int, actionType int, value int) {
	// Local variables
	g := p.Game
	t := g.Table
	variant := variants[t.Options.VariantName]
	clue := NewClue(actionType, value) // Convert the incoming data to a clue object
...
func (p *GamePlayer) DrawCard() {
	...
	if g.DeckIndex >= len(g.Deck) && !g.Options.AllOrNothing {
		...
		hLog.Infof("%v Marking to end the game on turn: %v", t.GetName(), g.EndTurn)
	}
}
