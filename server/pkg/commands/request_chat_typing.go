...
func commandChatTyping(ctx context.Context, s *Session, d *CommandData) {
	...
	if playerIndex == -1 && spectatorIndex == -1 {
		s.Warningf(
			"You are not playing or spectating at table %v, so you cannot report that you are typing.",
			t.ID,
		)
		return
	}
	if spectatorIndex == -1 && t.Replay {
		s.Warningf(
			"You are not spectating replay %v, so you cannot report that you are typing.",
			t.ID,
		)
		return
	}
