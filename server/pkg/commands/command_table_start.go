...
func tableStart(ctx context.Context, s *Session, d *CommandData, t *Table) {
	...
	shuffleDeck := true
	shufflePlayers := true
	seedPrefix := fmt.Sprintf(
		"p%vv%vs", // e.g. "p2v0s" for a 2-player no variant game
		len(t.Players),
		variant.ID,
	)
	if t.ExtraOptions.JSONReplay {
		...
		for looking {
			seedNum++
			g.Seed = fmt.Sprintf("%v%v", seedPrefix, seedNum)
	...
	if !t.ExtraOptions.NoWriteToDatabase {
		...
		for _, p := range t.Players {
			if p.Session != nil {
				p.Session.SetStatus(constants.StatusPlaying)
...
func emulateActions(ctx context.Context, s *Session, d *CommandData, t *Table) {
	...
	for i, action := range t.ExtraOptions.CustomActions {
		...
		if g.InvalidActionOccurred {
			...
			if s != nil {
				s.Warningf(
					"The action at index %v was not valid. Skipping all subsequent actions. Please report this error to an administrator.",
					i,
				)
			}
