...
func commandTableUnattend(ctx context.Context, s *Session, d *CommandData) {
	...
	if playerIndex == -1 && spectatorIndex == -1 {
		s.Warningf(
			"You are not playing or spectating at table %v, so you cannot unattend it.",
			t.ID,
		)
		return
	}
	if spectatorIndex == -1 && t.Replay {
		s.Warningf("You are not spectating replay %v, so you cannot unattend it.", t.ID)
		return
	}
...
func tableUnattendSpectator(ctx context.Context, s *Session, d *CommandData, t *Table, j int) {
	...
	if t.Replay && len(t.Spectators) == 0 {
		...
		deleteTable(t)
		hLog.Infof("Ended replay %v because everyone left.", t.ID)
		return
	}
