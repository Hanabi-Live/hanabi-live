...
func commandTableRestart(ctx context.Context, s *Session, d *CommandData) {
	...
	if !t.Replay || !t.Visible {
		s.Warningf(
			"Table %v is not a shared replay, so you cannot send a restart action.",
			t.ID,
		)
		return
	}
	...
	for _, sp := range t.Spectators {
		if sp.Session == nil {
			...
			s.Warning("One of the spectators is currently reconnecting. Please try restarting again in a few seconds.")
			return
		}
	...
	if len(playerSessions) != len(t.Players) {
		s.Warning("Not all of the players from the original game are in the shared replay, so you cannot restart the game.")
		return
	}
...
func tableRestart(
	...
	for _, s2 := range playerSessions {
		if len(tables.GetTablesUserPlaying(s2.UserID)) > 0 {
			s.Warningf(
				"You cannot restart the game because \"%v\" is already playing in another game.",
				s2.Username,
			)
			return
		}
	...
	if t.InitialName == "" {
		...
		if len(match) != 0 {
			...
			gameNumberString := match[0][2]
			if v, err = strconv.Atoi(gameNumberString); err != nil {
				hLog.Errorf(
					"Failed to convert the game number of \"%v\" to an integer: %v",
					gameNumberString,
					err,
				)
				s.Error("Something went wrong when restarting the game. Please report this error to an administrator.")
				return
			} else {
				gameNumber = v
			}
		...
		tableNameSuffix := fmt.Sprintf(" (#%v)", gameNumber)
	...
	if t2 == nil {
		hLog.Errorf(
			"Failed to find the newly created table of \"%v\" in the table map.",
			newTableName,
		)
		s.Error("Something went wrong when restarting the game. Please report this error to an administrator.")
		return
	...
	path := fmt.Sprintf("/replay/%v", t.ExtraOptions.DatabaseID)
	...
	msg := fmt.Sprintf("The game has been restarted (from game %v).", link)
