...
func (g *Game) End(ctx context.Context, d *CommandData) {
	...
	hLog.Infof("%v Ended with a score of: %v", t.GetName(), g.Score)
	...
	for _, p := range t.Players {
		if p.Session != nil {
			p.Session.SetStatus(constants.StatusLobby)
	...
	if err := g.WriteDatabase(); err != nil {
		hLog.Errorf("Failed to record the game to the database: %v", err)
		return
	}
	...
	if v, err := models.Seeds.GetNumGames(g.Seed); err != nil {
		hLog.Errorf("Failed to get the number of games on seed \"%v\": %v", g.Seed, err)
		return
	} else {
...
func (g *Game) WriteDatabase() error {
	...
	if v, err := models.Games.Insert(row); err != nil {
		return fmt.Errorf("failed to insert the game row: %w", err)
	} else {
		...
		if t.Options.DetrimentalCharacters {
			if gp.Character == "n/a" {
				...
				if v, ok := characters[gp.Character]; !ok {
					return fmt.Errorf(
						"the character of \"%v\" does not exist in the characters map",
						gp.Character,
					)
				} else {
					characterID = v.ID
	...
	if err := models.GameParticipants.BulkInsert(gameParticipantsRows); err != nil {
		return fmt.Errorf("failed to insert the game participant rows: %w", err)
	}
	...
	if len(gameActionRows) > 0 {
		if err := models.GameActions.BulkInsert(gameActionRows); err != nil {
			return fmt.Errorf("failed to insert the game action rows: %w", err)
		}
	}
	...
	if len(gameParticipantNotesRows) > 0 {
		if err := models.GameParticipantNotes.BulkInsert(gameParticipantNotesRows); err != nil {
			hLog.Errorf("Failed to insert the game participants notes rows: %v", err)
	...
	if len(chatLogRows) > 0 {
		if err := models.ChatLog.BulkInsert(chatLogRows); err != nil {
			hLog.Errorf("Failed to insert the chat message rows: %v", err)
	...
	if len(gameTagsRows) > 0 {
		if err := models.GameTags.BulkInsert(gameTagsRows); err != nil {
			hLog.Errorf("Failed to insert the tag rows: %v", err)
	...
	if err := models.Seeds.UpdateNumGames(g.Seed); err != nil {
		hLog.Errorf("Failed to update the number of games in the seeds table: %v", err)
	...
	hLog.Infof(
		"Finished core database actions for table %v (to database ID %v).",
		t.ID,
...
func (g *Game) WriteDatabaseStats() {
	...
	for _, p := range t.Players {
		...
		if v, err := models.UserStats.Get(p.UserID, variant.ID); err != nil {
			hLog.Errorf("Failed to get the stats for user \"%v\": %v", p.Name, err)
			continue
		} else {
			userStats = v
		}
		...
		if err := models.UserStats.Update(p.UserID, variant.ID, userStats); err != nil {
			hLog.Errorf("Failed to update the stats for user \"%v\": %v", p.Name, err)
			continue
		}
	}
...
func (t *Table) ConvertToSharedReplay(ctx context.Context, d *CommandData) {
	...
	t.Name = fmt.Sprintf("Shared replay for game #%v", t.ExtraOptions.DatabaseID)
	...
	for _, p := range t.Players {
		...
		if !p.Present {
			if p.UserID == t.OwnerID && (p.Session == nil || p.Session.ms.IsClosed()) {
				...
				ownerOffline = true
				hLog.Infof(
					"\"%v\" was the owner of the game and they are offline; passing the leader to someone else.",
					p.Name,
				)
			}
			continue
		...
		hLog.Infof("Converted \"%v\" to a spectator.", p.Name)
	}

	// End the shared replay if no-one is left
	if len(t.Spectators) == 0 {
		deleteTable(t)
		hLog.Infof("Ended table #%v because no-one was present when the game ended.", t.ID)
		return
	}
	...
	if ownerOffline {
		...
		for _, p := range t.Players {
			if p.Present {
				t.OwnerID = p.UserID
				hLog.Infof("Set the new leader to be: %v", p.Name)
				break
			}
		}
		...
		if t.OwnerID == -1 {
			...
			hLog.Infof(
				"All players are offline; set the new leader to be: %v",
				t.Spectators[0].Name,
			)
		}
	}
	...
	for _, sp := range t.Spectators {
		...
		if sp.Session != nil {
			sp.Session.SetStatus(constants.StatusSharedReplay)
