...
func NewSession() *Session {
	return &Session{
		...
		Data: &SessionData{
			Status:             constants.StatusLobby, // By default, new users are in the lobby
...
func (s *Session) Emit(command string, d interface{}) {
	...
	if dataJSON, err := json.Marshal(d); err != nil {
		hLog.Errorf("Failed to marshal data when writing to a WebSocket session: %v", err)
		return
	} else {
...
func (s *Session) Warning(msg string) {
	hLog.Infof("Warning - %v - %v", msg, s.Username)
...
func (s *Session) Error(msg string) {
	hLog.Infof("Error - %v - %v", msg, s.Username)
