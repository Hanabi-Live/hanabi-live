...
func discordInit() {
	...
	if len(discordToken) == 0 {
		hLog.Info("The \"DISCORD_TOKEN\" environment variable is blank; aborting Discord initialization.")
		return
	}
	discordGuildID = os.Getenv("DISCORD_GUILD_ID")
	if len(discordGuildID) == 0 {
		hLog.Info("The \"DISCORD_GUILD_ID\" environment variable is blank; aborting Discord initialization.")
		return
	}
	if _, err := strconv.Atoi(discordGuildID); err != nil {
		logger.Fatalf(
			"Failed to convert the \"discordGuildID\" environment variable of \"%v\" to an integer: %v",
			dbPort,
			err,
		)
	}
	...
	if len(discordChannelSyncWithLobby) == 0 {
		hLog.Info("The \"DISCORD_CHANNEL_SYNC_WITH_LOBBY\" environment variable is blank; aborting Discord initialization.")
		return
	}
	if _, err := strconv.Atoi(discordChannelSyncWithLobby); err != nil {
		logger.Fatalf(
			"Failed to convert the \"discordChannelSyncWithLobby\" environment variable of \"%v\" to an integer: %v",
			dbPort,
			err,
		)
	}
...
func discordConnect() {
	...
	botUsername := fmt.Sprintf("Bot %v", discordToken)
	if v, err := discordgo.New(botUsername); err != nil {
		hLog.Errorf("Failed to create a Discord session: %v", err)
		return
	} else {
		discord = v
	}
	...
	if err := discord.Open(); err != nil {
		hLog.Errorf("Failed to open the Discord session: %v", err)
		return
	}
...
func discordReady(s *discordgo.Session, event *discordgo.Ready) {
	hLog.Infof("Discord bot connected with username: %v", event.User.Username)
...
func discordSend(to string, username string, msg string) {
	...
	if _, err := discord.ChannelMessageSendComplex(to, messageSendData); err != nil {
		...
		hLog.Infof("Failed to send \"%v\" to Discord: %v", fullMsg, err)
		return
	}
}
