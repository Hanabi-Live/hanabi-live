  ...
  if (image instanceof SVGSVGElement) {
    if (image.childNodes && image.childNodes.length > 1) {
      ...
      if (group) {
        ...
        group.setAttribute(
          "transform",
          transformDirective + " scale(" + scaleW + ", " + scaleH + ")",
        );
        parent.appendChild(group);
      }
    }
...
ctx.prototype.__applyFilter = function () {
  ...
  if (filter != null) {
    var match =
      /^drop-shadow\(([^p]+)px\s+([^p]+)px\s+([^p]+)px\s+([\s\S]+?)\)$/.exec(
        filter,
      );
    if (match != null) {
      var dx = match[1];
      ...
      var feDropShadowElement = this.__createElement(
        "feDropShadow",
        {
          dx: dx,
          dy: dy,
      ...
      var filterElement = this.__createElement(
        "filter",
        {
          id: id,
        },
