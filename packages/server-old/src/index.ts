...
const borderText = `+ ${"-".repeat(startText.length - 1)} +`;
...
app.get("/", (req, res) => {
  res.render("index", {
    // This will look for "src/views/index.ejs"
    ...
    websocketURL: `${req.protocol}${
      req.get("X-Forwarded-Proto") === "https" ? "s" : ""
...
app.use(
  serveFavicon(path.join(__dirname, "..", "public", "img", "favicon.png")),
);
...
websocketServer.on("connection", (socket) => {
  ...
  socket.on("message", (data) => {
    try {
      ...
      logger.warn(
        "Encountered an error while processing message.",
        err.message,
      );
