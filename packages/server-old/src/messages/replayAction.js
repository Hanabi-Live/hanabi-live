...
exports.step1 = (socket, data) => {
  ...
  if (!game.sharedReplay) {
    logger.warn(
      `User "${socket.username}" tried to perform a replay action on non-replay replay game #${data.gameID}.`,
    );
  ...
  if (socket.userID !== game.owner) {
    logger.warn(
      `User "${socket.username}" tried to perform an action on shared replay #${data.gameID}, but they were not the owner.`,
    );
  ...
  if (!Number.isInteger(data.type) || data.type < 0 || data.type > 1) {
    logger.warn(
      `User "${socket.username}" tried to perform an invalid replay action of type "${data.type}" on shared replay #${data.gameID}.`,
    );
  ...
  if (!Number.isInteger(data.value) || data.value < 0) {
    logger.warn(
      `User "${socket.username}" tried to specify an invalid replay action with value "${data.value}" on shared replay #${data.gameID}.`,
    );
  ...
  for (const userID of Object.keys(game.spectators).filter(
    (id) => Number(id) !== game.owner,
  )) {
