...
exports.step1 = (socket, data) => {
  ...
  if (game.players.length < 2) {
    logger.warn(
      `messages.startGame was called for game #${data.gameID}, but it does not have at least 2 players.`,
    );
  ...
  if (game.running) {
    logger.warn(
      `messages.startGame was called for game #${data.gameID}, but it was already started.`,
    );
...
function step3(socket, data) {
  ...
  logger.info(
    `Using seed ${game.seed}, timed is ${game.timed}, reorderCards is ${game.reorderCards}.`,
  );
  ...
  logger.info(
    "(cards are dealt to a player until their hand fills up before moving on to the next one)",
  );
