...
export function sudokuCanStillBePlayed(
    ...
    for(const stackStart of possibleStarts) {
        ...
        // For this, note that we can compare the difference of our card and the start with the
        // longest play sequence starting at the start, thereby checking if the specified rank is
        // included.
...
export function getMaxScorePerStack(
        ...
        if (couldIncrement) {
            if (localSuitIndex === unassignedSuits.length - 1) {
                ...
                curAssignment.forEach( (assignedStackStartIndex, assignedLocalSuitIndex) => {
                    const value = maxPartialScores[unassignedSuits[assignedLocalSuitIndex]!]![possibleStackStarts[assignedStackStartIndex]! -1]!;
                    assignmentVal += value;
                ...
                if (assignmentVal > bestAssignmentSum) {
                    ...
                    // If the values are the same, we want to update if the assignment is
    ...
    unassignedSuits.forEach((unassignedSuit, unassignedLocalSuitIndex) => {
        // Note the '??' here, since it can be that there is actually no feasible assignment. In
        // this case, these values are still undefined at this point, so we replace them by 0.
        maxScorePerStack[unassignedSuit] = bestAssignment[unassignedLocalSuitIndex] ?? 0;
    });
