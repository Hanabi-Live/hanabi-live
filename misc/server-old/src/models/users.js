...
exports.getUser = (socket, data, done) => {
  ...
  db.query(sql, values, (error, results, fields) => {
    ...
    if (results.length === 0) {
      ...
      error = new Error(
        `Got ${results.length} rows in the "users" table for: ${data.username}`,
      );
      done(error, socket, data, null);
      return;
    } else {
...
exports.getStats = (socket, data, done) => {
  ...
  db.query(sql, values, (error, results, fields) => {
    ...
    if (results.length === 0) {
      error = new Error(
        `There was no rows in the "users" table for the user ID of: ${data.userID}`,
      );
      done(error, socket, data);
      return;
    } else if (results.length !== 1) {
      error = new Error(
        `Got ${results.length} rows in the "users" table for the user ID of: ${data.userID}`,
      );
      done(error, socket, data);
      return;
    }
