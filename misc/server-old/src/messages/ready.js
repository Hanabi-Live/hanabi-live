...
exports.step1 = (socket, data) => {
  ...
  if (!(data.gameID in globals.currentGames) && socket.status !== "Replay") {
    logger.warn(
      `User "${data.username}" tried to ready for game #${data.gameID} with status ${socket.status}, but that game does not exist.`,
    );
