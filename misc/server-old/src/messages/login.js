...
exports.step1 = (socket, data) => {
  ...
  if (data.username.length > maxLength) {
    logger.warn(
      `User "${data.username}" supplied an excessively long username with a length of ${data.username.length}.`,
    );
  ...
  if (data.username.length < minLength) {
    logger.warn(
      `User "${data.username}" supplied an excessively short username with a length of ${data.username.length}.`,
    );
...
function step2(error, socket, data) {
  ...
  // "x-forwarded-for" is added by Heroku; note that it has to be in lowercase
  ...
  if (data.userID === null) {
    ...
    logger.info(
      `User "${data.username}" supplied an incorrect password of: ${data.password}`,
    );
...
function step4(socket, data) {
  ...
  if (socket.userID in globals.connectedUsers) {
    logger.info(
      `User "${socket.username}" logged in but was already connected; logging the existing user out.`,
    );
  ...
  logger.info(
    `User "${data.username}" logged in from IP "${data.ip}". (${
      Object.keys(globals.connectedUsers).length
    } now connected.)`,
  );
  ...
  // (Keldon sends these, but they seem rather useless, so we won't bother)
  /*
    socket.emit('message', {
        type: 'chat',
        resp: {
