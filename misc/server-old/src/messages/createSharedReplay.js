...
function step2(error, socket, data) {
  ...
  if (data.variant === null) {
    logger.warn(
      `User "${socket.username}" requested to start a shared replay for game #${data.gameID}, which does not exist.`,
    );
  ...
  logger.info(
    `User "${socket.username}" created a new shared replay: #${data.gameID}`,
  );
