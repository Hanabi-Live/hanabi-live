...
// (this has to be in every file that accesses any environment variables)
...
function discordMessage(message) {
  ...
  if (!("author" in message)) {
    logger.error(
      'Failed to parse the "author" field from the Discord message.',
    );
    return;
  } else if (!("channel" in message)) {
    logger.error(
      'Failed to parse the "channel" field from the Discord message.',
    );
    return;
  } else if (!("id" in message.channel)) {
    logger.error(
      'Failed to parse the "channel.id" field from the Discord message.',
    );
    return;
  } else if (!("username" in message.author)) {
    logger.error(
      'Failed to parse the "author.username" field from the Discord message.',
    );
    return;
  } else if (!("discriminator" in message.author)) {
    logger.error(
      'Failed to parse the "author.discriminator" field from the Discord message.',
    );
    return;
  } else if (!("bot" in message.author)) {
    logger.error(
      'Failed to parse the "author.bot" field from the Discord message.',
    );
    return;
  } else if (!("content" in message)) {
    logger.error(
      'Failed to parse the "content" field from the Discord message.',
    );
    return;
  }
...
exports.send = (from, username, message) => {
  ...
  // In Discord, text inside single asterisks are italicized and text inside
