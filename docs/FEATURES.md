...
* If two groups of Hanabi players want to compete against each other, then there are a few ways to play a non-randomly generated deal:
  ...
  * Start a game with a name of `!deal [deal]` to play a deal specified by a text file. The text file must already be present on the server in the `specific_deals` directory. If necessary, you can contact an administrator to upload a new text file. For example: `!deal showmatch-jan-2050-game-1`
