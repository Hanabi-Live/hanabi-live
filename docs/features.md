...
- If `players` is omitted or empty, a seed such as `p4v0s1` supplies the player count, and stock names (Alice, Bob, Cathy, Donald, Emily, Frank) are used. If the seed does not identify the player count, provide the `players` array explicitly.
